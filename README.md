...
A current review requires the listed revision to equal the observed head, a readable published request and verdict ref, and the explicit `verifier_run_id` binding attested by Forest when publishing the verdict. An approve decision with that binding establishes verification and displays its Verifier Run ID; Ledger rows are optional enrichment, not a verification prerequisite. Canopy still rejects conflicting Run roles or immutable work provenance in observed review/history/status rows. Legacy verdicts without the binding remain displayable as “verdict published; verifier identity unbound (legacy)” and never become verified through time-window correlation, even when exactly one Run overlaps publication. A changed head makes the previous join stale until the new revision publishes its own refs. Missing verdicts stay absent; malformed verdicts stay unreadable. Checks are separately published evidence, not a replacement for a verdict. GitHub comments are optional human sugar and are not read or required by Canopy.
